...
products = Task.objects.filter(category="Out Door")

# (9)***Order/Sort Objects by id
leastToGreatest = Task.objects.all().order_by('id')
greatestToLeast = Task.objects.all().order_by('-id')


# (10) Returns all products with tag of "Sports": (Query Many to Many Fields)
productsFiltered = Tasks.objects.filter(tags__name="Sports")
